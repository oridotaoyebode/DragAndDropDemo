...
namespace Digiterra.DragDrop.Demo.iOS
{
    public class TouchRecognizer : UIGestureRecognizer
    {
        ...
        Effects.Touch.TouchEffect touchEffect;
        bool capture;
        ...
        public override void TouchesBegan(NSSet touches, UIEvent evt)
        {
            ...
            foreach (UITouch touch in touches.Cast<UITouch>())
            {
                ...
                FireEvent(this, id, TouchType.Pressed, touch, true);
        ...
        public override void TouchesMoved(NSSet touches, UIEvent evt)
        {
            ...
            foreach (UITouch touch in touches.Cast<UITouch>())
            {
                ...
                if (capture)
                {
                    FireEvent(this, id, TouchType.Moved, touch, true);
                }
                else
                {
                    ...
                    {
                        FireEvent(idToTouchDictionary[id], id, TouchType.Moved, touch, true);
                    }
                }
            }
        }
        ...
        public override void TouchesEnded(NSSet touches, UIEvent evt)
        {
            ...
            foreach (UITouch touch in touches.Cast<UITouch>())
            {
                ...
                if (capture)
                {
                    FireEvent(this, id, TouchType.Released, touch, false);
                }
                else
                {
                    CheckForBoundaryHop(touch);
                    ...
                    if (idToTouchDictionary[id] != null)
                    {
                        FireEvent(idToTouchDictionary[id], id, TouchType.Released, touch, false);
                    }
                }
        ...
        public override void TouchesCancelled(NSSet touches, UIEvent evt)
        {
            ...
            foreach (UITouch touch in touches.Cast<UITouch>())
            {
                ...
                if (capture)
                {
                    FireEvent(this, id, TouchType.Cancelled, touch, false);
                }
                else if (idToTouchDictionary[id] != null)
                {
                    FireEvent(idToTouchDictionary[id], id, TouchType.Cancelled, touch, false);
                }
                idToTouchDictionary.Remove(id);
            }
        }
        ...
        void CheckForBoundaryHop(UITouch touch)
        {
            ...
            if (recognizerHit != idToTouchDictionary[id])
            {
                if (idToTouchDictionary[id] != null)
                {
                    FireEvent(idToTouchDictionary[id], id, TouchType.Exited, touch, true);
                }
                if (recognizerHit != null)
                {
                    FireEvent(recognizerHit, id, TouchType.Entered, touch, true);
                }
                idToTouchDictionary[id] = recognizerHit;
            }
        }

        void FireEvent(TouchRecognizer recognizer, long id, TouchType actionType, UITouch touch, bool isInContact)
        {
            ...
            Action<Element, TouchEventArgs> onTouchAction = recognizer.touchEffect.OnTouchAction;

            // Call that method
            onTouchAction(recognizer.element,
                new TouchEventArgs(id, actionType, xfPoint, isInContact));
        }
    }
}